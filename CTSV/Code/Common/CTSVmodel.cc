...
#include <oct.h>
#include <oct-rand.h>
DEFUN_DLD(CTSVmodel, args, ,"CTSVmodel"){
    // parameters of model
    ...
    Array <float> normalshocks;
    Array <float> uniformshocks;
    ...
    ColumnVector e1s, e2s, e3s, e4s, e5s;
    e1s = octave_rand::vector((n+burnin)*M*MMM+100);
    e2s = octave_rand::vector((n+burnin)*M*MMM+100);
    e3s = octave_rand::vector((n+burnin)*M*MMM+100);
    e5s = octave_rand::vector((n+burnin)*M*MMM+100);
