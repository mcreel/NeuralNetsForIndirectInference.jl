...
# below here, no need to touch anything for
# the ordinary 2 hidden layer specication
...
output = InnerProductLayer(name="aggregator", output_dim=OutputDim, tops=[:output], bottoms=[:pred2] )
...
datatest = MemoryDataLayer(batch_size=testbatchsize, data=Array[XT,YT])
