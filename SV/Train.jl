...
#loss(x,y) = SmoothQRobj(model(x),y, x, [0.05, 0.5, 0.95]) # + 0.1*L2penalty(θ) # QR with regularization
loss(x,y) = QRobj(model(x),y, [0.05, 0.5, 0.95]) # + 0.1*L2penalty(θ) # QR with regularization
function monitor(e)
...
for i = 1:5000
    ...
    if current < bestsofar
        ...
        prettyprint(reshape(round.(accuracy,digits=3),1,3))
    end
end
end 
main();
